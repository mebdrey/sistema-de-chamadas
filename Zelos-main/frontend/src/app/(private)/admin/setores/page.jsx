...
export default function Setores() {
    ...
    const usuariosFiltrados = Object.entries(setores)
        ...
        .filter((usuario) => {
            ...
            return (
                ...
                usuario.funcao.toLowerCase().includes(termo) || usuario.id.toString().includes(termo)  // caso queira buscar por id também
            );
        });
    ...
    function formatarLabel(str) {
        ...
        const correcoes = { "auxiliar limpeza": "Auxiliar de Limpeza", "apoio tecnico": "Apoio Técnico", "tecnico": "Técnico" };
                ...
                <div className="flex items-center justify-between flex-column md:flex-row flex-wrap space-y-4 md:space-y-0 p-4 ">
                                        ...
                                        <li key={nomeSetor}>
                                            <div className="flex p-2 rounded-sm hover:bg-gray-100 dark:hover:bg-gray-600">
                                                <div className="flex items-center h-5">
                                                    <input id={`checkbox-${nomeSetor}`} type="checkbox" checked={!!setoresSelecionados[nomeSetor]} onChange={() => toggleSetor(nomeSetor)} className="w-4 h-4 text-[#7F56D8] bg-gray-100 border-gray-300 rounded-sm focus:ring-[#7F56D8] dark:focus:ring-blue-600 dark:ring-offset-gray-700 dark:focus:ring-offset-gray-700 focus:ring-2 dark:bg-gray-600 dark:border-gray-500" />
                                                </div>
                    ...
                    <form className="flex items-center" onSubmit={(e) => e.preventDefault()}>
                        ...
                        <div className="relative w-80">
                            ...
                            <input type="text" id="simple-search" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-[#7F56D8] focus:border-[#7F56D8] block w-full ps-10 p-2.5" placeholder="Pesquisar por usuário" value={busca} onChange={(e) => setBusca(e.target.value)} />
                        </div>
                    </form>
                </div>
                {/* Dropdown botão */}
                    ...
                    <table className="w-full text-sm text-left text-gray-500 ">
                        ...
                        <tbody className='w-full'>
                            {usuariosFiltrados.length === 0 && (
                                <tr className="w-full">
                                    <td colSpan="3" className="w-full px-6 py-4 text-center"> Nenhum usuário para o tipo selecionado.</td>
                                </tr>
                            )}
