...
export default function MeuPerfil() {
    ...
    //     const res = await fetch("/api/editarPerfil/foto", {method: "POST", body: formData });
    //     if (res.ok) {alert("Foto atualizada!");}
    //else {
    ...
    const handleSubmit = async (e) => {
        ...
        Object.keys(formData).forEach(key => { if (formData[key] === undefined) delete formData[key]; });
        ...
        if (Object.keys(formData).length === 0) {
            ...
            const response = await fetch('http://localhost:8080/editarPerfil', {
                method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(formData), credentials: 'include'
            });
        ...
        <div className="p-4 h-screen w-full">
            <div className="p-4 mt-14">
                ...
                <div className="flex flex-wrap gap-6">
                    <button type="button" className="bg-violet-600 h-10 text-white font-medium py-2 px-4 rounded hover:bg-violet-700 transition mt-5" data-bs-toggle="modal" data-bs-target="#staticBackdrop">
                        Editar perfil
                    </button>
