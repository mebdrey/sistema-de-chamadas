...
export default function ChamadosTecnico({ downloadMode = 'open' // 'open' ou 'download'
  ...
  const iniciarApontamento = async () => {
    ...
    try {
      const response = await fetch('http://localhost:8080/criar-apontamento', {
        ...
        credentials: 'include',
        body: JSON.stringify({ chamado_id: chamadoSelecionado.id, descricao })
      });
  ...
  const finalizarApontamento = async (id) => {
    try {
      const response = await fetch('http://localhost:8080/finalizar-apontamento', {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ apontamento_id: id })
      });
  ...
  const enviarMsg = async (tipo = 'tecnico') => {
    ...
    try {
      const endpoint = tipo === 'tecnico' ? 'http://localhost:8080/tecnico-enviar-msg' : 'http://localhost:8080/enviar-msg';

      const response = await fetch(endpoint, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ idChamado: chamadoSelecionado.id, conteudoMsg: conteudo }),
      });

      if (!response.ok) throw new Error('Erro ao enviar mensagem');

      // Atualiza mensagens
      const res = await fetch(`http://localhost:8080/chat?idChamado=${chamadoSelecionado.id}`, { method: 'GET', credentials: 'include' });
  ...
  useEffect(() => {
    const fetchMensagens = async () => {
      ...
      try {
        const response = await fetch(`http://localhost:8080/chat?idChamado=${chamadoSelecionado.id}`, { method: 'GET', credentials: 'include' });
  ...
  const finalizarChamado = async () => {
    ...
    try {
      ...
      const resp = await fetch('http://localhost:8080/finalizar-chamado', {
        method: 'PATCH', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ chamado_id: chamadoSelecionado.id })
      });
  ...
  const podeFinalizar = !!(
    chamadoSelecionado && normalizarId(chamadoSelecionado.status_chamado) === "em-andamento" &&
    effectiveCurrentUserId && String(chamadoSelecionado.tecnico_id) === effectiveCurrentUserId
  );

  const podeGerarRelatorio = !!(
    chamadoSelecionado && normalizarId(chamadoSelecionado.status_chamado) === "concluido" &&
    effectiveCurrentUserId && String(chamadoSelecionado.tecnico_id) === effectiveCurrentUserId
  );
  ...
  function formatarLabel(str) {
    if (!str) return ""
    const correcoes = { media: "Média", manutencao: "Manutenção", apoio_tecnico: "Apoio Técnico" };
          ...
          <section>
            ...
            <div id="default-tab-content">
              {statusAbas.map((status) => {
                ...
                let chamadosFiltrados = chamados
                  .filter((c) => {
                    const correspondeStatus = statusIdAtivo === "todos" || normalizarId(c.status_chamado) === statusIdAtivo;

                    const correspondeBusca = busca.trim() === "" || c.assunto.toLowerCase().includes(busca.toLowerCase()) || c.descricao.toLowerCase().includes(busca.toLowerCase()) || String(c.id).includes(busca);

                    const correspondeSetor = setoresSelecionados.length === 0 || setoresSelecionados.includes(mapaTipoIdParaTitulo[c.tipo_id]);

                    const correspondePrioridade = prioridadesSelecionadas.length === 0 || prioridadesSelecionadas.includes(c.prioridade_id);
                      ...
                      chamadosFiltrados.map((chamado) => ( //CARD DOS CHAMADOS
                        <div key={chamado.id} onClick={() => {
                          ...
                          {chamado.status_chamado === 'pendente' && (
                            <button onClick={(e) => { e.stopPropagation(); pegarChamado(chamado.id); }} className="cursor-pointer mt-4 bg-violet-500 hover:bg-violet-600 text-white text-sm px-4 py-2 rounded-lg" >
                              Pegar chamado
                            </button>
                          )}
                        </div>
            ...
            {isOpen && chamadoSelecionado && (
              (chamadoSelecionado.status_chamado === 'pendente') ? (
                <div id="drawer-right-example" className={`fixed top-0 right-0 z-99 h-screen p-4 overflow-y-auto transition-transform border-l border-gray-200 dark:border-gray-600 bg-white w-80 dark:bg-gray-800 ${isOpen ? "translate-x-0" : "translate-x-full"}`} tabIndex="-1" aria-labelledby="drawer-right-label" >
                ...
                <div id="drawer-right-example" className={`fixed top-0 right-0 z-99 h-screen overflow-y-auto transition-transform border-l border-gray-200 dark:border-neutral-700 bg-[#F8FAFB] w-full dark:bg-gray-800 ${isOpen ? "translate-x-0" : "translate-x-full"}`} tabIndex="-1" aria-labelledby="drawer-right-label" >
                  ...
                  <div className="w-full p-4 bg-white dark:bg-gray-900">
                    <h5 id="drawer-right-label" className="inline-flex items-center text-base poppins-semibold text-gray-500 dark:text-gray-400">
                      <svg className="w-4 h-4 me-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20"><path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5ZM9.5 4a1.5 1.5 0 1 1 0 3 1.5 1.5 0 0 1 0-3ZM12 15H8a1 1 0 0 1 0-2h1v-3H8a1 1 0 0 1 0-2h2a1 1 0 0 1 1 1v4h1a1 1 0 0 1 0 2Z" /></svg>
                      Detalhes do chamado
                    </h5>
                    <button type="button" onClick={() => setIsOpen(false)} className="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center dark:hover:bg-gray-600 dark:hover:text-white" >
                      <svg className="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14"><path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" /></svg>
                        ...
                        {!expandido && (
                          <div className="absolute bottom-0 left-0 w-full h-16 bg-gradient-to-t from-gray-50 dark:from-gray-900 lg:hidden flex items-end justify-center">
                            <button className="mb-2 flex items-center gap-2 px-4 py-1 bg-white dark:bg-gray-800 border rounded-full shadow-md" onClick={() => setExpandido(true)}>
                              <span className="text-sm text-gray-600 dark:text-gray-300">Ver mais</span><span className="w-4 h-4">▼</span>
                            </button>
                          </div>
                        )}
                        {expandido && (
                          <div className="md:hidden flex justify-center">
                            <button className="flex items-center gap-2 px-4 py-1 bg-white dark:bg-gray-800 border rounded-full shadow-md" onClick={() => setExpandido(false)}>
                              <span className="text-sm text-gray-600 dark:text-gray-300">Ver menos</span><span className="w-4 h-4">▲</span>
                            </button>
                          </div>
                        )}
                      </div>
                        ...
                        <ol className="relative bg-white dark:bg-gray-900 rounded-lg border-s border-gray-300 dark:border-gray-600 mb-10">
                          {apontamentos.map((a) => (
                            <li key={a.id} className="mb-10 py-4 ms-4 dark:bg-gray-900">
                              ...
                              <time className="mb-1 text-sm text-gray-500">{new Date(a.comeco).toLocaleString('pt-BR')}</time>
                              <h3 className="text-lg poppins-semibold dark:text-white">{a.fim ? 'Apontamento finalizado' : 'Apontamento em andamento'}</h3>
                              <p className="text-gray-700 dark:text-white">{a.descricao}</p>
                              {a.fim && (<p className="text-sm text-gray-500 mt-1">Encerrado em {new Date(a.fim).toLocaleString('pt-BR')}</p>)}
                            </li>
                          ))}
                        </ol>
                        {/* {!apontamentoAtivo && (
