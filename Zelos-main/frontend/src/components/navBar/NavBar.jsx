...
const SideBar = ({ user, setUser, userType, navFechada, setNavFechada }) => {
    ...
    return (
        <>
                        ...
                        <div className='flex flex-row gap-10'>
                            ...
                            <div className="flex items-center">
                                <div className="flex items-center ms-3">
                                    <div>
                                        <button type="button" aria-expanded={dropdownUserOpen} onClick={() => setDropdownUserOpen(!dropdownUserOpen)} className="flex text-sm bg-gray-800 rounded-full focus:ring-4 focus:ring-gray-300 dark:focus:ring-gray-600">
                                    ...
                                    {dropdownUserOpen && (
                                        <div className="z-50 absolute right-0 top-10 my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-sm shadow-sm dark:bg-gray-700 dark:divide-gray-600" id="dropdown-user">
                                            ...
                                            <ul className="py-1" role="none">
                                                ...
                                                <li>
                                                    <button className="w-full text-left block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-600 dark:hover:text-white" role="menuitem" onClick={(e) => {
                                                        e.preventDefault();
                                                        handleLogout();
                                                    }}>Sair</button>
                                                </li>
            ...
            {/* itens da sidebar */}
            <aside id="logo-sidebar" className={`fixed top-0 left-0 z-40 h-screen pt-20 transition-all bg-white border-r border-gray-200 dark:bg-gray-800 dark:border-gray-700 ${navFechada ? 'w-16' : 'w-64'}`} aria-label="Sidebar">
                <div className={`flex px-2 mb-5 ${navFechada ? 'justify-center' : 'justify-end'}`}>
                    <button onClick={sidebar} data-drawer-target="logo-sidebar" data-drawer-toggle="logo-sidebar" aria-controls="logo-sidebar" className="text-gray-500 hover:text-gray-800 dark:text-gray-400 dark:hover:text-white transition-colors">
                        {navFechada ? (
                        ...
                        {links.map((link) => (
                            <li key={link.href}>
                                <a href={link.href} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                                    {link.icon}
                                    <span className={"ml-3 transition-all duration-200 " + (navFechada ? "opacity-0 w-0 overflow-hidden" : "opacity-100 w-auto")}>
                                        {link.label}
                                    </span>
                                </a>
                            </li>
